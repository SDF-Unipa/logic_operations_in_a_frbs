# =============================================================================
# TOPSIS — example 1 (positive Latin languages case)
#
# LITERATURE
# ----------
# Primary method:
...
#   Rows = LLM alternatives; columns = primal dimension scores IT, PT, EN.
...
#   IT, PT, EN → benefit (+1): higher score on each language is preferable.
#
# Criterion weights  (Hwang & Yoon 1981, §3.1 "Step 3"):
#   Uniform: w = 1/3 for each criterion.
#
...
# type = +1 (benefit): higher score is preferable for all three dimensions.
# ---------------------------------------------------------------------------
CRITERIA = [
    {"name": "IT", "description": "Italian score",    "type":  1},  # benefit: higher Italian performance is better
    {"name": "PT", "description": "Portuguese score", "type":  1},  # benefit: higher Portuguese performance is better
    {"name": "EN", "description": "English score",    "type":  1},  # benefit: higher English performance is better
]

# Raw mean scores per model per dimension.
# Source: scores_simplified.json  (@10% mean values)
SCORES: dict[str, dict[str, float]] = {
    "MichelRosselli/apertus": {"IT": 10.1, "PT":  9.2, "EN": 10.8},
    "gemma4:e4b":             {"IT": 13.4, "PT": 14.7, "EN": 16.8},
    "mistral-small3.2:24b":   {"IT": 16.7, "PT": 15.6, "EN": 18.4},
    "nemotron-cascade-2:30b": {"IT": 16.4, "PT": 20.4, "EN": 22.2},
}
...
def run() -> dict[str, dict]:
    ...
    # Criterion types: +1 benefit for IT, PT, EN  (Hwang & Yoon 1981, §3.1 "Step 4")
...
def main() -> None:
    ...
    print(r"\centering")
    print(r"\caption{TOPSIS ranking for the positive Latin languages case (uniform weights, benefit: IT, PT, EN).}")
    print(r"\label{tab:topsis-latin-languages-ranking}")
