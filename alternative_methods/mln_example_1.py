# =============================================================================
# Markov Logic Networks (MLN) — example 1 (positive Latin languages case)
#
# LITERATURE
# ----------
# Original MLN paper:
...
#   Each IF-THEN rule is represented as the clause ¬A1 ∨ … ∨ ¬An ∨ C,
#   which is violated only when all antecedents hold and the head is false.
#   The weight w controls how much the distribution disfavours violating worlds:
#     • w → +∞: the rule becomes a hard constraint (violated worlds get P = 0).
#     • w = 0:  the rule is ignored (uniform prior over worlds).
#     • w = 2.0 (this file): moderate soft constraint.  A world that satisfies
#               the rule is e^2.0 ≈ 7.4× more probable than one that violates it,
#               all else being equal.
#   Richardson & Domingos (2006, §5) learn weights via pseudo-likelihood
#   maximisation from a relational database.  Here weights are set uniformly
#   to 2.0 because no labelled data is available; the value can be treated as
#   a prior belief in the rules' reliability.
#
...
#   Richardson & Domingos use MC-SAT for large domains.  With 6 input +
#   4 output atoms the ground network has only 2^10 = 1024 worlds, so exact
#   inference by full enumeration is tractable and preferred for correctness.
#
# Score aggregation — expected value:
#   The marginal vector {P(weight_very_high), …, P(weight_very_low)} is
#   collapsed to a scalar using the same expected-value formula as in
#   problog_example_1.py and routine_example_1.py:
...
# Maps the linguistic label key used in MEMBERSHIPS (e.g. "IT_High") to the
# atom name used in RULES antecedents (e.g. "it_high").
...
def main() -> None:
    ...
    print(r"\centering")
    print(r"\caption{MLN ranking for the positive Latin languages case (rule weight = " + str(RULE_WEIGHT) + r").}")
    print(r"\label{tab:mln-latin-languages-ranking}")
