# Larsen inference for Example 1 with the Gödel (minimum) T-norm.
# T_Gödel(a, b) = min(a, b)  — the weakest T-norm; fires at the bottleneck
# antecedent's membership value rather than their product.
...
def godel_min(values: list[float]) -> float:
    # Gödel T-norm: T(a, b) = min(a, b).
    # The rule fires at the strength of its weakest antecedent match.
    return min(values)
...
for model, values in MEMBERSHIPS.items():
    ...
    for rule in RULES:
        alpha = godel_min([values[key] for key in rule["antecedents"]])
