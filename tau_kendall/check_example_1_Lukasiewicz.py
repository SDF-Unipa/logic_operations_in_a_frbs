# Larsen inference for Example 1 with the Łukasiewicz T-norm.
# T_Łukasiewicz(a, b) = max(0, a + b − 1)  — the strongest bounded T-norm;
# returns 0 unless both antecedents are simultaneously strong.
# Comparing rankings against check_example_1.py measures sensitivity to the
# choice of T-norm conjunction operator.
import csv
...
def lukasiewicz(values: list[float]) -> float:
    # Łukasiewicz T-norm applied left-to-right: T(a, b) = max(0, a + b − 1).
    # With n antecedents this generalizes to max(0, Σ μ_k − (n − 1)).
    result = values[0]
    for v in values[1:]:
        result = max(0.0, result + v - 1.0)
    return result
...
for model, values in MEMBERSHIPS.items():
    ...
    for rule in RULES:
        alpha = lukasiewicz([values[key] for key in rule["antecedents"]])
