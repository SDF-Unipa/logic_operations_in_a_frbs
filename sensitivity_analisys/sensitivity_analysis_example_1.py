"""
Sensitivity analysis for routine_example_1.
...
METADATA_DIR = Path(__file__).with_name("example_1_metadata")
...
def _run_batch(
    ...
    for run_idx in range(N_RUNS):
        ...
        print(
            f"  run {run_idx + 1:02d}  tau={tau_stat.statistic:.4f}  p={tau_stat.pvalue:.4f}"
            f"  -> {run_path.relative_to(METADATA_DIR)}"
        )
...
def run() -> None:
    ...
    for pct in tqdm(PERTURBATION_PCTS, desc="tolerances", unit="tol"):
        ...
        print(f"=== {pct*100:g}% perturbation ({label}) ===")
        ...
        print(f"  mean tau={np.mean(taus):.4f}  std={np.std(taus):.4f}\n")
    ...
    print(f"tolerances summary -> {tol_path}")
